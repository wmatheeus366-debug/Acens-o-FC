...
(function () {
  ...
  function resolveSlot(g, slot) {
    ...
    if (slot.comp === "EST") {
      ...
      if (slot.stage === "SF") {
        ...
        return mkFix(g, "EST", est.name + " · Semifinal", [k.sfMine.a, k.sfMine.b], { stage: "SF", knock: true, decisive: true, tie: k.sfMine, cup: "ESTK" });
      }
      if (slot.stage === "F") {
    ...
    if (slot.comp === "LIGA") {
      ...
      const late = slot.round >= L.nRounds - 2;
      return mkFix(g, "LIGA", L.name + " · Rodada " + slot.round, pair, {
        round: slot.round, decisive: isClassic || late || isRivalDuel, classic: isClassic, rivalDuel: isRivalDuel
      });
    }
    if (slot.comp === "CDB" || slot.comp === "COPA") {
      ...
      return mkFix(g, slot.comp, cup.name + " · " + STAGE_NAMES[slot.stage], pair, { stage: slot.stage, knock: true, decisive: true, tie: tie, cup: slot.comp, stageIdx: nxt.stageIdx });
    }
    if (slot.comp === "CONTI") {
      ...
      return mkFix(g, C.id, C.name + " · " + STAGE_NAMES[stage], home ? [p.clubId, oppId] : [oppId, p.clubId], { stage: stage, knock: true, decisive: true, conti: true, ko: slot.ko });
    }
    if (slot.comp === "SEL") {
      const nat = D.NATIONS[p.nat];
      if (slot.kind === "elim") {
        return mkNatFix(g, "SEL", "Eliminatórias · " + nat.name, slot.opp, slot.home, { decisive: true, sel: "elim" });
      }
      const T = S.sel;
      if (!T || T.kind === "elim") return null;
      if (slot.phase === "G") {
        if (!T.alive) return null;
        return mkNatFix(g, T.kind, T.name + " · Grupo, jogo " + slot.n, slot.opp, slot.home, { decisive: true, sel: "tourG", n: slot.n });
      }
  ...
  function peekSlot(g, slot) {
    ...
    if (slot.comp === "EST") {
      ...
      return { comp: "EST", label: est.name + " · " + (slot.stage === "SF" ? "Semifinal" : "Final"), oppName: null, home: null, decisive: true };
    }
    if (slot.comp === "LIGA") {
      const L = S.comps.LIGA;
      const pair = (L.rounds[slot.round - 1] || []).find(function (pr) { return pr[0] === p.clubId || pr[1] === p.clubId; });
    ...
    if (slot.comp === "CDB" || slot.comp === "COPA") {
      ...
      return { comp: slot.comp, label: cup.name + " · " + STAGE_NAMES[slot.stage], oppName: null, home: null, decisive: true };
    }
    if (slot.comp === "CONTI") {
      ...
      return { comp: C.id, label: C.name + " · " + STAGE_NAMES[stage], oppName: opp ? oppObj(g, opp).name : null, oppId: opp || null, home: slot.ko % 2 === 0, decisive: true };
    }
    if (slot.comp === "SEL") {
      const nat = D.NATIONS[p.nat], T = S.sel;
      if (slot.kind === "elim") return { comp: "SEL", label: "Eliminatórias · " + nat.name, oppName: (slot.opp || "").slice(4), home: slot.home, decisive: true };
      if (slot.phase === "G") return { comp: "SEL", label: (T ? T.name : "Torneio") + " · Grupo, jogo " + slot.n, oppName: (slot.opp || "").slice(4), home: slot.home, decisive: true };
      return { comp: "SEL", label: (T ? T.name : "Torneio") + " · " + STAGE_NAMES[slot.stage], oppName: null, home: null, decisive: true };
    }
    return null;
  }
