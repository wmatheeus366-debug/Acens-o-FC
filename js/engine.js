...
(function () {
  ...
  function endSeason(g) {
    ...
    // volta a ex-clube: mutuamente exclusivo dos outros gatilhos de mercado, só a
    // partir dos 31 anos, chance de 30% por temporada elegível (não todo ano — vira
    // mais especial, como uma ligação de agente de verdade)
    let homecomingOffers = null;
    if (!retiring && !offers && !loanOffer && p.age >= 31) {
      const formerIds = formerClubs(g);
      if (formerIds.length && U.chance(0.3, U.rngFor(g.seed, "homecomingroll", g.year))) {
        homecomingOffers = makeHomecomingOffers(g, formerIds);
      }
    }
  ...
  // lista de clubes distintos pelos quais o jogador já passou, excluindo o atual —
  // não existe "clube de origem" separado no jogo, p.career é a única fonte disso.
  function formerClubs(g) {
    const p = g.player, seen = {}, out = [];
    p.career.forEach(function (c) {
      if (c.clubId !== p.clubId && !seen[c.clubId]) { seen[c.clubId] = true; out.push(c.clubId); }
    });
    return out;
  }
  // volta a ex-clube: diferente do empréstimo (1 proposta só), aqui é "pra qual clube
  // você quer voltar" — mostra TODOS os ex-clubes elegíveis de uma vez.
  function makeHomecomingOffers(g, formerIds) {
    const p = g.player, rng = U.rngFor(g.seed, "homecoming", g.year);
    return formerIds.map(function (clubId) {
      const cl = D.CLUBS[clubId], lg2 = leagueOf(g, clubId);
      return {
        clubId: clubId, name: cl.name, league: D.LEAGUES[lg2] ? D.LEAGUES[lg2].short : lg2,
        salary: Math.round(calcSalary(p.overall, cl.str, lg2) * U.rf(0.85, 1.1, rng) / 500) * 500,
        years: U.ri(1, 2, rng), role: p.overall >= cl.str - 4 ? "estrela" : "titular"
      };
    });
  }
