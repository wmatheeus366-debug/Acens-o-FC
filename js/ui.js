...
(function () {
  ...
  // partida ao vivo segue sozinha (sem precisar clicar "Continuar" a cada lance) —
  // pedido do usuário: mais cinematográfico, "cenas passando". Guarda o timer pendente
  // pra poder cancelar (partida encerrada/decisão aparece/usuário clica "Continuar" pra
  // adiantar manualmente) — nunca deixa 2 avanços agendados ao mesmo tempo.
  let liveAutoTimer = null;
  function clearLiveAuto() {
    if (liveAutoTimer) { clearTimeout(liveAutoTimer); liveAutoTimer = null; }
  }
  function scheduleLiveAuto(fn, ms) {
    clearLiveAuto();
    liveAutoTimer = setTimeout(function () {
      liveAutoTimer = null;
      if (!CQ.state.live) return; // partida já encerrada nesse meio-tempo
      fn();
    }, ms);
  }
  ...
  function liveStep() {
    clearLiveAuto(); // chamado tanto pelo timer quanto por clique manual — nunca 2x agendado
    const live = CQ.state.live;
    ...
    if (live.phase === "decision") {
      ...
      // a partida segue sozinha — botão fica só como atalho pra quem quer adiantar na
      // hora. Pausa mais longa quando vários lances chegaram juntos, pra dar tempo de
      // ler o feed/ver os splashes antes do próximo trecho.
      foot.innerHTML = `<button class="btn btn-block" onclick="CQ.ui.liveStep()">Adiantar &rsaquo;</button>`;
      scheduleLiveAuto(liveStep, Math.min(5200, 2200 + revealed.length * 900));
    }
    const box = $("#cq-overlay .overlay-box");
  ...
  function renderShootout() {
    ...
    if (running) {
      ...
      scheduleLiveAuto(shootReveal, 2600); // disputa também segue sozinha, um pouco mais devagar (mais tensão)
    } else {
  ...
  function shootReveal() {
    clearLiveAuto(); // chamado tanto pelo timer quanto por clique manual
    const live = CQ.state.live;
